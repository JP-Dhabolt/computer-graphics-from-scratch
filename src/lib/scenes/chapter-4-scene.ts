import { Blue, Green, Red, Yellow } from '$lib/colors';
import { Ambient, Directional, Point } from '$lib/rayTracer/lights';
import { Scene } from '$lib/scene';
import { Sphere } from '$lib/shapes/sphere';

export const scene = new Scene([new Ambient(0.2), new Point([2, 1, 0], 0.6), new Directional([1, 4, 4], 0.2)]);
scene.addSphere(new Sphere([0, -1, 3], 1, Red, 500, 0.2));
scene.addSphere(new Sphere([2, 0, 4], 1, Blue, 500, 0.3));
scene.addSphere(new Sphere([-2, 0, 4], 1, Green, 10, 0.4));
scene.addSphere(new Sphere([0, -5001, 0], 5000, Yellow, 1000, 0.5));
